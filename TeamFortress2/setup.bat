...
IF NOT EXIST "%GameRoot%tf\custom\logs"			( MKLINK /D "%GameRoot%tf\custom\logs"       "%~dp0%tf\custom\logs" )

:: symlink files
IF NOT EXIST "%GameRoot%tf\benchmark_test.dem"	( MKLINK "%GameRoot%tf\benchmark_test.dem"   "%~dp0%tf\benchmark_test.dem" )

:: run compile scripts
CD "%~dp0%tf\custom\captions\resource"
CALL "compile.bat"
CD "%~dp0%tf\custom\crosshairs\materials\vgui\replay\thumbnails"
CALL compile.bat
